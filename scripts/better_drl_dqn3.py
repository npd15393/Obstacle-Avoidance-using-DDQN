...
class ReplayMemory(object):
    ...
    def append(self, state, action, reward, done):
        ...
        """
        assert (
            state.shape == self._state_shape
        ), "Invalid state shape (required: %s, got: %s)" % (
            self._state_shape,
            state.shape,
        )
    ...
    def sample(self, size):
        ...
        count, pos, history_len, terminals = (
            self._count - 1,
            self._pos,
    ...
    def minibatch(self, size):
        ...
        pre_states = np.array(
            [self.get_state(index) for index in indexes], dtype=np.float32
        )
        post_states = np.array(
            [self.get_state(index + 1) for index in indexes], dtype=np.float32
        )
...
class DeepQAgent(object):
    ...
    def __init__(
        ...
        nb_actions,
        gamma=0.99,
        explorer=LinearEpsilonAnnealingExplorer(1, 0.1, 100000),
        learning_rate=0.001,
        momentum=0.95,
        minibatch_size=32,
        memory_size=500000,
        ...
        l_sgd = adam(
            self._action_value_net.parameters,
            lr_schedule,
            momentum=m_schedule,
        ...
        self._metrics_writer = (
            TensorBoardProgressWriter(freq=1, log_dir="metrics", model=criterion)
            if monitor
            else None
        )
        self._learner = l_sgd
        self._trainer = Trainer(
            criterion, (criterion, None), l_sgd, self._metrics_writer
        )
    ...
    def train(self):
        ...
        if agent_step >= self._train_after:
            if (agent_step % self._train_interval) == 0:
                pre_states, actions, post_states, rewards, terminals = (
                    self._memory.minibatch(self._minibatch_size)
                )
    ...
    def _plot_metrics(self):
        ...
        if len(self._episode_q_means) > 0:
            ...
            self._metrics_writer.write_value(
                "Mean Q per ep.", mean_q, self._num_actions_taken
            )

        if len(self._episode_q_stddev) > 0:
            std_q = np.asscalar(np.mean(self._episode_q_stddev))
            self._metrics_writer.write_value(
                "Mean Std Q per ep.", std_q, self._num_actions_taken
            )

        self._metrics_writer.write_value(
            "Sum rewards per ep.", sum(self._episode_rewards), self._num_actions_taken
        )
...
def interpret_action(action):
    step_meters = 5
    if action == 0:
        quad_pos_offset = (0, 0, 0)  # No position change
    elif action == 1:
        quad_pos_offset = (step_meters, 0, 0)  # Move +1 in X axis
    elif action == 2:
        quad_pos_offset = (-step_meters, 0, -180)  # Move -1 in X axis
    elif action == 3:
        quad_pos_offset = (0, step_meters, 90)  # Move +1 in Y axis
    elif action == 4:
        quad_pos_offset = (0, -step_meters, -90)  # Move -1 in Y axis

    return quad_pos_offset
...
def compute_reward(quad_state, quad_vel, collision_info, yaw_rate):
    ...
    if (
        (collision_info.position[b"x_val"] != 0)
        or (collision_info.position[b"y_val"] != 0)
        or (collision_info.position[b"z_val"] != 0)
    ):
...
def moveInHeading(client, velocity=1, heading=0, altitude=-5, duration=1):
    ...
    client.moveByVelocityZ(
        ...
        YawMode(False, yaw_err / pi),
    )  # Move 5,5,5 meter in heading mode
    (pitch, roll, yaw) = client.getPitchRollYaw()
    roll = math.degrees(roll)
    pitch = math.degrees(pitch)
    yaw = math.degrees(yaw)
...
def moveInGrid(client, offset=(0, 0, 0), velocity=1, altitude=-5):
    client.rotateToYaw(offset[2], 60, 5)
    quad_pose = client.getPosition()
    print(
        "QUAD_POS X: {0} Y: {1} Z: {2}".format(
            quad_pose.x_val, quad_pose.y_val, quad_pose.z_val
        )
    )
    X_pos = round(quad_pose.x_val + offset[0])
    Y_pos = round(quad_pose.y_val + offset[1])
    client.moveToPosition(X_pos, Y_pos, altitude, velocity)
    quad_pose = client.getPosition()
    print(
        "QUAD_POS X: {0} Y: {1} Z: {2}".format(
            quad_pose.x_val, quad_pose.y_val, quad_pose.z_val
...
if __name__ == "__main__":
    ...
    for episode in range(n_episodes):
        ...
        #    moveInGrid(client, (0,0,0))
        #    moveInGrid(client, (1,0,0))
        #    moveInGrid(client, (-1,0,180))
        #    moveInGrid(client, (0,1,90))
        #    moveInGrid(client, (0,-1,-90))
        ...
        responses = client.simGetImages(
            [ImageRequest(1, AirSimImageType.DepthPerspective, True, False)]
        )
        ...
        for steps in range(max_steps):
            ...
            quad_pos_offset = interpret_action(action)
            ...
            moveInGrid(client, quad_pos_offset, V, -5)
            ...
            reward = compute_reward(
                quad_pose, quad_vel, collision_info, quad_yaw_offset
            )

            # Check if the episode is done
            done = isDone(reward)
            print(
                "Episode {} Action {}  Reward {}  Done: {} ".format(
                    episode, action, reward, done
                )
            )
    ...
    print("******************************END***********************************")
    print(
        "The Training has been completed for {} episodes with model saved ".format(
            n_episodes
        )
    )
